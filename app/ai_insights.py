...
SYSTEM_PROMPT = (
    ...
    "- A shortfall risk only exists when the projected balance goes negative, or when a large expense lands before a covering income"
    " deposit with insufficient buffer to absorb it. A balance drop that remains comfortably positive is not a shortfall risk.\n"
    ...
    "- risk: a specific future event where the projected balance goes negative or a large expense lands before a covering income deposit"
    " with insufficient buffer to absorb it.\n"
    ...
    "  ]\n"
    "}\n\n"
    "The severity field is required for every insight. Use 'high' for risks driven by a specific "
    "identifiable transaction or timing conflict that could be acted on (e.g. a large expense "
    "landing before an income deposit). Use 'medium' for patterns that create stress but are "
    "harder to act on directly (e.g. recurring expenses clustering in the same window). "
    "Use 'low' for observations that are noteworthy but unlikely to require action.\n\n"
