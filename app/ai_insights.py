...
SYSTEM_PROMPT = (
    ...
    "Context inference (do this before generating insights):\n"
    "First, infer whether this account appears to represent:\n"
    "- personal finances\n"
    "- business finances\n"
    "- or a mixed-use account\n"
    "Use transaction names, recurring income sources, expense categories, and transaction patterns to determine the most likely "
    "context. Tailor all observations, patterns, recommendations, and tone accordingly. Do not state the inferred context "
    "explicitly in your output; let it shape word choice and category names instead.\n\n"
    "Input data format:\n"
    ...
    "Software, Professional Services, etc.). Pick category names appropriate to the inferred personal/business/mixed context.\n"
    "  * For each category, sum the absolute monthly value of its expenses. Convert each expense to a monthly equivalent using "
    "its frequency (weekly ≈ x4.33, biweekly ≈ x2.17, semimonthly ≈ x2, monthly ≈ x1, quarterly ≈ x0.333, semiannual ≈ x0.167, "
    "annual ≈ x0.0833, onetime ≈ counted once at its amount). Report each sum as a positive dollar figure rounded to the "
    "nearest dollar.\n"
    "  * Title: 'Expense breakdown by category' (or a similarly clear label).\n"
    "  * Description: a plain-text bulleted list with each category on its own line, prefixed with '• ', showing the category "
    "name and its monthly total. Example line: '• Housing: $2,400/mo'. Sort bullets by total in descending order and omit any "
    "category whose total rounds to $0. Do not add commentary before or after the list.\n"
    "  * Severity: 'low'.\n"
    "  * If there are no expense entries at all, omit this insight.\n\n"
